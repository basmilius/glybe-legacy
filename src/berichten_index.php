...
{
	$folderId = Glybe::Security($_GET['fid']);
} else {
	$folderId = 0;
}
$p = ((isset($_GET['p']) && is_numeric($_GET['p']) && $_GET['p'] > 0) ? $_GET['p'] : 1);

$bQuery = DB::Query("SELECT 1 FROM messages pm WHERE pm.folder_id = '" . $folderId . "' AND pm.user_to_id = '" . $user->Id . "' AND pm.state = 'open'");
				...
				<div class="inner">
					<form action="" method="post">
						<div class="psystem" style="float: left;">
							...
							{
								echo'<a href="/berichten/index?fid=' . $folderId . '&p=' . $value[1] . '"><div class="item">' . $value[0] . '</div></a>';
							}
							?>
							<div class="clear"></div>
						</div>
						<div class="selectbox" style="z-index: 1000; float: right;">
							<div class="caption">Met geselecteerde berichten..<div class="arrow"></div></div>
							<div class="dropdown" style="top: -80px;">
								<div class="item static">Markeren&nbsp;als..</div>
								<div class="item" onclick="Glybe.Berichten.Command('mark_as_readed');">&nbsp;&nbsp;&nbsp;&nbsp;Gelezen</div>
								<div class="item" onclick="Glybe.Berichten.Command('mark_as_unreaded');">&nbsp;&nbsp;&nbsp;&nbsp;Ongelezen</div>
								<div style="border-top: 1px solid #C6C6C6; margin: 5px;"></div>
								<div class="item static">Verplaatsen&nbsp;naar..</div>
								<div class="item" onclick="Glybe.Berichten.Command('move_to', 0);">&nbsp;&nbsp;&nbsp;&nbsp;Postvak&nbsp;IN</div>
								<?php
								$fQuery = DB::Query("SELECT * FROM `messages_folders` WHERE `user_id` = '" . $user->Id . "'");
								if(DB::NumRows($fQuery) === 0)
								{
									...
									while($fFetch = DB::Fetch($fQuery))
									{
										echo '<div class="item" onclick="Glybe.Berichten.Command(\'move_to\', ' . $fFetch['id'] . ');">&nbsp;&nbsp;&nbsp;&nbsp;' . str_replace(" ", "&nbsp;", htmlspecialchars($fFetch['caption'])) . '</div>';
									}
								}
								?>
								<div style="border-top: 1px solid #C6C6C6; margin: 5px;"></div>
								<div class="item" onclick="Glybe.Berichten.Command('remove');">&nbsp;&nbsp;&nbsp;&nbsp;Verwijderen</div>
							</div>
						</div>
						<div class="clear"></div>
						...
						<div style="border-top: 1px solid #C6C6C6; margin: 5px;"></div>
						<?php
						$myFolder = (DB::NumRowsQuery("SELECT 1 FROM messages_folders WHERE id = '" . $folderId . "' AND user_id = '" . $user->Id . "'") > 0);
						if(!$myFolder && $folderId != 0)
						{
							echo'<div class="error_notif error">Heuhj... Dit is jou map niet...</div>';
						} else {
							$messagesQuery = DB::Query("	SELECT
													...
													AND
														usb.user_id = ub.id
													AND
														pm.folder_id = '" . $folderId . "'
													AND
														pm.user_to_id = '" . $user->Id . "'
													AND
														pm.state = 'open'
													ORDER BY
														id DESC
							...
							if(DB::NumRows($messagesQuery) === 0)
							{
								...
								echo'	<table style="width: 100%;" border="0" cellspacing="0">
										...
										<td style="padding: 5px; width: 16px;"><input type="checkbox" onchange="var b = document.getElementsByClassName(\'pm_sel\'); for(var i in b) { b[i].checked = this.checked; }" /></td>
								...
								while($messagesFetch = DB::Fetch($messagesQuery))
								{
									...
									echo'	<tr onmouseover="this.style.background = \'#EEEFFF\';" onmouseout="this.style.background = \'#FFFFFF\';">
											<td style="padding: 5px; width: 16px;"><input type="checkbox" name="pm[' . $messagesFetch['id'] . ']" class="pm_sel" /></td>
											<td style="padding: 6px; width: 16px;"><div class="icon email"></div></td>
						...
						<div class="selectbox" style="z-index: 1000; float: right;">
							<div class="caption">Met geselecteerde berichten..<div class="arrow"></div></div>
							<div class="dropdown" style="bottom: 0px;">
								<div class="item static">Markeren&nbsp;als..</div>
								<div class="item" onclick="Glybe.Berichten.Command('mark_as_readed');">&nbsp;&nbsp;&nbsp;&nbsp;Gelezen</div>
								<div class="item" onclick="Glybe.Berichten.Command('mark_as_unreaded');">&nbsp;&nbsp;&nbsp;&nbsp;Ongelezen</div>
								<div style="border-top: 1px solid #C6C6C6; margin: 5px;"></div>
